...
qtyf = np.abs(fth[1].data['Roll_RA'][fiid] - 83.63)
idxf = np.where(qtyf == qtyf.min())[0][0] + fiid[0]
qf = fth[1].data['Q_SAT'][idxf]
Rf = quat2dc(qf)
rdid = idxf + 169
vd1 = iner2dc(np.deg2rad(fth[1].data['Roll_RA'][idxf]),np.deg2rad(fth[1].data['Roll_DEC'][idxf]))
vd2 = iner2dc(np.deg2rad(fth[1].data['Roll_RA'][rdid]),np.deg2rad(fth[1].data['Roll_DEC'][rdid]))
vds = np.cross(vd1.A1,vd2.A1) # y axis of ccsc
vds = vds/np.linalg.norm(vds) # GOT vds and vrs inverted !! FIX IT LASFRI TODO FIXME

siid = np.arange(7555,12940) #indices for RA scan 4th orbit
qtys = np.abs(sth[1].data['Roll_DEC'][siid] - 22.01)
idxs = np.where(qtys == qtys.min())[0][0] + siid[0]
qs = fth[1].data['Q_SAT'][idxs]
Rs = quat2dc(qs)
rrotn = 0.5*fth[1].data['Roll_ROT'][idxf] + 0.5*sth[1].data['Roll_ROT'][idxs]

vrs = np.array([0.0,0.0,1.0]) #x axis of collimator camera scan coordinate (ccsc)
vzs = np.cross(vrs,vds) # z axis of ccsc
#Note above would have been fine enough to give unit vec, if vrs and vds are
#normal to each other. But they are not !! What do I do ? - Wed 27 Dec 2017 09:12:06 PM IST 
...
time = hdllc[1].data['TIME'] + (hdllc[1].header['TIMEZERI'] + \
